...
from tensorflow.python.keras.layers import Conv2D, Dense, DepthwiseConv2D
...
from nets.CSPdarknet import Focus, SiLU
from nets.yolo import get_train_model, yolo_body
from nets.yolo_training import get_lr_scheduler, yolo_loss
...
def apply_quantization(layer):  # 在这里决定 哪些层采用int4 量化 哪些层采用int8量化  如果不量化则注释掉
    quantize_layer_list = {}

    # yolov5 总共有xxx层卷积层
    quantize_layer_list['backbone.stem.conv.conv'] = 4
    quantize_layer_list['backbone.dark2.0.conv'] = 8

    # quantize_layer_list = ['backbone.stem.conv.conv']
    #     , 'backbone.dark2.1.cv1.conv', 'backbone.dark2.1.m.0.cv1.conv',
    # 'backbone.dark2.1.m.0.cv2.conv', 'backbone.dark2.1.cv2.conv', 'backbone.dark2.1.cv3.conv', 'backbone.dark3.0.conv', 'backbone.dark3.1.cv1.conv',
    # 'backbone.dark3.1.m.0.cv1.conv', 'backbone.dark3.1.m.0.cv2.conv', 'backbone.dark3.1.m.1.cv1.conv', 'backbone.dark3.1.m.1.cv2.conv',
    # 'backbone.dark3.1.m.2.cv1.conv', 'backbone.dark3.1.m.2.cv2.conv', 'backbone.dark3.1.cv2.conv']#, , , , , , , , , ,  ]

    quantize_config4 = MDQC_4bits(num_bits=4)
    quantize_config8 = MDQC_8bits(num_bits=8)

    # 全部层量化
    if 'conv' in layer.name and 'bn' not in layer.name:
        return tfmot.quantization.keras.quantize_annotate_layer(layer, quantize_config4)
        # return tfmot.quantization.keras.quantize_annotate_layer(layer, quantize_config8)

    # 指定卷积层名称量化
    # if layer.name in quantize_layer_list:
    #     return tfmot.quantization.keras.quantize_annotate_layer(layer, quantize_config4 if quantize_layer_list[layer.name] == 4 else quantize_config8)
...
class DefaultDenseQuantizeConfig(tfmot.quantization.keras.QuantizeConfig):
    ...
    def set_quantize_weights(self, layer, quantize_weights):
        ...
        print("called set quantize weights")
...
class MDQC_4bits(DefaultDenseQuantizeConfig):
    def __init__(self, num_bits=4):
        super(MDQC_4bits, self).__init__(num_bits)
        self.num_bits = num_bits
...
class MDQC_8bits(DefaultDenseQuantizeConfig):
    def __init__(self, num_bits=8):
        super(MDQC_8bits, self).__init__(num_bits)
        self.num_bits = num_bits
...
if __name__ == "__main__":
    ...
    #------------------------------------------------------#
    train_annotation_path   = 'dataset/2017_train.txt'
    val_annotation_path     = 'dataset/2017_val.txt'
    ...
    Q_ENABLE = True
    model_ = model
    # with tf.compat.v1.keras.utils.custom_object_scope({"get_anchors_and_decode": get_anchors_and_decode, 'yolo_loss': yolo_loss}):
    if Q_ENABLE:
        with quantize_scope({'MDQC_4bits': MDQC_4bits, 'MDQC_8bits': MDQC_8bits, "Focus": Focus, "SiLU": SiLU}):

            annotated_model = tf.keras.models.clone_model(
                model_, clone_function=apply_quantization,
            )
