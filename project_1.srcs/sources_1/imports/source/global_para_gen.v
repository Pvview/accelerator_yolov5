...
(
	...
	always@(posedge clk) begin
		case(ofm_send_sel)
			2'b00: begin                      // 如果ofm_send_sel�?00
				no_pool_ofm_addr_start<=0;    // 将no_pool_ofm_addr_start设置�?0
				no_pool_ofm_addr_end<=conv_addr_len;   // 将no_pool_ofm_addr_end设置为conv_addr_len
				//pool_ofm_addr_start<=0;       // 将pool_ofm_addr_start设置�?0
				//pool_ofm_addr_end<=pool_addr_len;       // 将pool_ofm_addr_end设置为pool_addr_len
			end
			2'b01: begin                      // 如果ofm_send_sel�?01
				no_pool_ofm_addr_start<=conv_col;       // 将no_pool_ofm_addr_start设置为conv_col
				no_pool_ofm_addr_end<=conv_addr_len;    // 将no_pool_ofm_addr_end设置为conv_addr_len
				//pool_ofm_addr_start<=pool_col;          // 将pool_ofm_addr_start设置为pool_col
				//pool_ofm_addr_end<=pool_addr_len;       // 将pool_ofm_addr_end设置为pool_addr_len
			end
			2'b10: begin                      // 如果ofm_send_sel�?10
				no_pool_ofm_addr_start<=0;    // 将no_pool_ofm_addr_start设置�?0
				no_pool_ofm_addr_end<=conv_addr_len-conv_col;   // 将no_pool_ofm_addr_end设置为conv_addr_len减去conv_col
				//pool_ofm_addr_start<=0;       // 将pool_ofm_addr_start设置�?0
				//pool_ofm_addr_end<=pool_addr_len-pool_col;       // 将pool_ofm_addr_end设置为pool_addr_len减去pool_col
			end
			2'b11: begin                      // 如果ofm_send_sel�?11
				no_pool_ofm_addr_start<=conv_col;       // 将no_pool_ofm_addr_start设置为conv_col
				no_pool_ofm_addr_end<=conv_addr_len-conv_col;   // 将no_pool_ofm_addr_end设置为conv_addr_len减去conv_col
				//pool_ofm_addr_start<=pool_col;          // 将pool_ofm_addr_start设置为pool_col
				//pool_ofm_addr_end<=pool_addr_len-pool_col;       // 将pool_ofm_addr_end设置为pool_addr_len减去pool_col
	...
	always@(posedge clk) begin
	        ...
	        begin                            // 如果pool_enable�?0
			ofm_addr_start<=no_pool_ofm_addr_start;  // 将ofm_addr_start设置为no_pool_ofm_addr_start
			ofm_addr_end<=no_pool_ofm_addr_end;      // 将ofm_addr_end设置为no_pool_ofm_addr_end
		end
