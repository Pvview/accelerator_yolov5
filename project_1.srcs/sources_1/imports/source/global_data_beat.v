...
(
	...
	assign conv_col_mult_2=(conv_col<<1); //鐩稿綋浜庝箻浠?2
	...
	///////////////////////////////////////////
	reg [15:0] addr_cnt; //鍦板潃璁℃暟鍣?
	reg [8:0] col_cnt;  //鍒楄鏁板櫒
	reg row_cnt      ;
	...
	if (cov_sel)
	       if(cov_stride_sel)
		      if(col_cnt<2 || (addr_cnt<conv_col_mult_2 || addr_cnt>=conv_addr_len) || row_cnt) begin //卷积行数小于2，或者卷积行数小于卷积总行数的2倍，或者地址计数器大于卷积地址计数器
		    	 global_zero<=1'b1;
		      end else begin
		    	 global_zero<=1'b0;
		      end
		      ...
		      if(col_cnt<2 || (addr_cnt<conv_col_mult_2 || addr_cnt>=conv_addr_len)) begin //卷积行数小于2，或者卷积行数小于卷积总行数的2倍，或者地址计数器大于卷积地址计数器
		    	 global_zero<=1'b1;
		      end else begin
		    	 global_zero<=1'b0;
		      end
	else       
		 if(( addr_cnt>=conv_addr_len )) begin //卷积行数小于2，addr_cnt<5'd7 || addr_cnt>=conv_addr_len + 4'd7
		    	 global_zero<=1'b1;
		      end else begin
		    	 global_zero<=1'b0;
		      end     
	...
	if(cov_sel)
	  if(cov_stride_sel)
                if(addr_cnt==conv_col)
			         global_valid<=1'b1;
                else if (addr_cnt>conv_col && addr_cnt<addr_len_add_conv_col_add_1)
                    if (~row_cnt)
		...
		if(addr_cnt>=5'd3 && addr_cnt<addr_len_add_conv_col_add_1) begin
			global_valid<=1'b1;
		 end else begin
			global_valid<=1'b0;
		end
	...
	reg valid_delay_11;
	///////////////////////////////////////
	wire valid_delay_cov5;
	reg valid_delay_cov6 ;
	reg valid_delay_cov7 ;
	...
	////////////////////////////////////////
	com_shift_reg
	#(.DEPTH(5),.WIDTH(1),.SRL_STYLE_VAL("srl_reg"))
	u_com_shift_reg_cov1
	(
		.clk(clk),
		.si(global_valid),
		.so(valid_delay_cov5)
	...
	always@(posedge clk) begin
		...
		valid_delay_cov6 <= valid_delay_cov5;
		valid_delay_cov7 <= valid_delay_cov6;
		
	end
	assign acc_read_en =    (cov_sel)   ?   valid_delay_9  : valid_delay_cov5;
	assign acc_write_en=    (cov_sel)   ?   valid_delay_11 : valid_delay_cov7;
	...
	always@(posedge clk) begin
		if(shutdown) begin
			base_out_addr<=0;
		end else if(~cov_sel && valid_delay_cov5)
			 base_out_addr<=base_out_addr+1;     
		  else if(valid_delay_9 && cov_sel) 
			 base_out_addr<=base_out_addr+1;
		  else
